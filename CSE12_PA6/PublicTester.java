...
public class PublicTester {
    ...
    public void testAddFirst() {
        ...
        assertEquals("6 should have been inserted into index 2",
                Integer.valueOf(6), deque.data[2]);
        ...
        assertEquals("Index 5 should not have changed", Integer.valueOf(6),
                deque.data[5]);
    }
    ...
    public void testAddLast() {
        ...
        assertEquals("-1 should have been inserted into index 5",
                Integer.valueOf(-1), deque.data[5]);
        assertEquals("Index 2 should not have changed", Integer.valueOf(4),
                deque.data[2]);
        assertEquals("Index 3 should not have changed", Integer.valueOf(5),
                deque.data[3]);
        assertEquals("Index 4 should not have changed", Integer.valueOf(1),
                deque.data[4]);
    }
    ...
    public void testRemoveFirst() {
        ...
        assertEquals("Rear should not change after calling removeFirst", 3,
                deque.rear);
        assertEquals("Index 2 should remain unchanged", Integer.valueOf(4),
                deque.data[2]);
        assertEquals("Index 3 should remain unchanged", Integer.valueOf(1),
                deque.data[3]);
        assertEquals("Index 1 should have been set to null", null,
                deque.data[1]);
    }
    ...
    public void testRemoveLast() {
        ...
        assertEquals("Index 1 should remain unchanged", Integer.valueOf(3),
                deque.data[1]);
        assertEquals("Index 2 should remain unchanged", Integer.valueOf(4),
                deque.data[2]);
        assertEquals("Index 3 should have been set to null", null,
                deque.data[3]);
    }
    ...
    public void testStackPop() {
        ...
        if (res.intValue() == 1 || res.intValue() == 3) {
            assertEquals("Size should have decremented", 2,
                    stack.theStack.size);
            if (stack.theStack.peekFirst().intValue() != 2 &&
                    stack.theStack.peekLast().intValue() != 2) {
    ...
    public void testStackPeek() {
        ...
        if (res.intValue() == 1 || res.intValue() == 3) {
            assertEquals("Size should not have decremented", 3,
                    stack.theStack.size);
            if (stack.theStack.peekFirst().intValue() != 1 &&
                    stack.theStack.peekLast().intValue() != 3) {
    ...
    public void testQueueDequeue() {
        ...
        if (res.intValue() == 1) {
            if (queue.theQueue.peekFirst().intValue() != 2) {
                fail("Next element to remove should be 2");
            }
        } else if (res.intValue() == 3) {
            if (queue.theQueue.peekLast().intValue() != 2) {
                fail("Next element to remove should be 2");
            }
        } else {
            fail("An element on one of the ends should have been returned");
    ...
    public void testQueuePeek() {
        ...
        if (res.intValue() == 1 || res.intValue() == 3) {
            assertEquals("Size should not have decremented", 3,
                    queue.theQueue.size);
            if (queue.theQueue.peekFirst().intValue() != 1 &&
                    queue.theQueue.peekLast().intValue() != 3) {
                fail("Elements on either end should not have changed");
            }
        } else {
            fail("An element on one of the ends should have been returned");
